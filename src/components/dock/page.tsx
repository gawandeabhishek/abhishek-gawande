...
export default function Dock() {
  return (
    <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50">
      <div className="flex items-end gap-1.5 bg-white/5 backdrop-blur-xl  rounded-2xl px-3 py-2.5">
        {dockItems.map((item, i) =>
          item === null ? (
            <div key={`sep-${i}`} className="w-px h-7 bg-white/10 mx-1" />
          ) : (
            <a
              key={item.title}
              href={item.href}
              ...
              title={item.title}
              className="w-11 h-11 rounded-xl bg-white/2 backdrop-blur-sm flex items-center justify-center text-xl
